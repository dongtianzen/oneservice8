...
class DashpageBlockGenerator extends ControllerBase {
  ...
  function contentRenderHeader() {
    $output = '';
    $output .= '<div class="panel-header program-implementation-overview">';
      $output .= '<span>{{block.top.value}}</span>';
      $output .= '<md-menu>';
        $output .= '<span id="save-button-3-{{block.blockId}}" ng-click="openMenu($mdOpenMenu, $event)" class="fa fa-angle-down float-right font-size-14 padding-12 cursor-pointer"></span>';
  ...
  function contentRenderTable() {
    ...
    $output .= '<div class="content-render-table-wrapper">';
      ...
      $output .= '<div class={{block.middle.middleMiddle.middleMiddleMiddleClass}}>';
        $output .= '<div data-ng-controller="AngularDataTables" class="row margin-top-24">';
            ...
            $output .= '</thead>';
              $output .= '<tbody>';
                $output .= '<tr data-ng-repeat="tableRow in block.middle.middleMiddle.middleMiddleMiddle.value.tbody">';
                  $output .= '<td data-ng-repeat="tableColumn in tableRow">';
                    $output .= '{{tableColumn}}';
                  $output .= '</td>';
                $output .= '</tr>';
              $output .= '</tbody>';
  ...
  function contentRenderMultiContainer() {
    ...
    $output .= '<div class="content-render-multicontainer-wrapper">';
      ...
      $output .= '<div class={{block.middle.middleMiddle.middleMiddleMiddleClass}}>';
        $output .= '<div data-ng-repeat="chart in block.middle.middleMiddle.middleMiddleMiddle" class="{{chart.value.chartClass}}">';
          $output .= '<canvas width="600" height="400" flexxiachartjs options="{{chart.value.chartOptions}}" type="{{chart.value.chartType}}" id="{{chart.value.chartId}}" data="{{chart.value.chartData}}"></canvas>';
          $output .= '<div class="row margin-top-n-10 margin-bottom-12 padding-left-24 font-size-13 text-align-left">';
            $output .= '<span>{{chart.value.chartTitle}}</span>';
          $output .= '</div>';
        $output .= '</div>';
      $output .= '</div>';
  ...
  function topWidgetsFixed($nid = NULL) {
    $output = '';
    $output .= '<div data-ng-repeat="widget in landingPageData.fixedSection">';
      $output .= '<div data-ng-switch="widget.type">';
        /*
         * Widget type "widgetOne"
         */
        $output .= '<div data-ng-switch-when="widgetOne">';
          $output .= '<div class="{{widget.class}} margin-top-6">{{widget.class}}';
            $output .=  '<md-content layout-padding="">';
              $output .= '<div id="block-id-1-{{$index}}" class="dashpage-square-number-wrapper border-radius-3 {{widget.value.header.class}}">';
                ...
                $output .= '<div class="dashpage-square-number-top border-1-e7e7e7-n padding-12">';
                  $output .= '<div class="padding-top-6 font-size-16">{{widget.value.header.valueOne.value}}';
                    $output .= '<md-menu>';
                      $output .= '<span id="save-button-1-{{$index}}" ng-click="openMenu($mdOpenMenu, $event)" class="fa fa-angle-down float-right padding-12 padding-top-0 cursor-pointer"></span>';
                      $output .= '<md-menu-content width="3">';
  ...
  function contentBlockMultiTabs() {
    ...
    $output .= '<md-tabs md-selected="selectedIndex" md-dynamic-height="" md-border-bottom="">';
      $output .= '<md-tab ng-repeat="block in block.middle.value" label="{{block.title}}">';
        $output .= '<div data-ng-switch="block.contentType">';
  ...
  function contentBlockMaster() {
    $output = '';
    $output .= '<div id="block-id-3-{{block.blockId}}" class="panel">';
